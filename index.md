...
1.  **TEFCA-level Patient Sharing Controls and Notifications via Discoverable Interfaces:**
    ...
    *   These controls and notification preferences, once set through a TEFCA-recognized interface, must be propagated and honored by all TEFCA network participants. Supported controls must include:
        ...
        *   **Opt-In Network Access Notifications:** An option for individuals to receive notifications for TEFCA-based queryies or disclosures of their health records. )These could also serve as the trigger for "Ask Me First" approvals.)

2.  **Patient-Accessible Network Audit Logs:**
...
*   [Empower Individuals with Transparency and Control Over TEFCA Data Sharing](#req_tefca_individual_transparency_control): Provide API-accessible audit logs and network-level patient controls (opt-out, "ask me first," freeze access).
